...
: number) {
  ...
  return response
    .data
    .map((e
    : EpisodeInterface) => ({id: e.id, name: e.name, season: e.season, number: e.number}));
}
...
: EpisodeInterface[]): void {
  ...
  for (let episode of episodes) {
    ...
    const $episode = $(
      `<li><strong>${episode.id}. ${episode.name} </strong>(season ${episode.season}, ep. ${episode.number}</li>
      `
    );
